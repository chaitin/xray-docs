---
title: 'reverseType'
description: '`reverseType`类型主要是返回了反连平台提供的各个信息，现假设变量名为 `reverse`（需要先使用 `newReverse()` 生成实例)'
---

<Info>本文中的`domain`将拿`example.com`作为示例，`ip`将拿`10.1.1.1`作为示例，端口将拿`8899`作为示例</Info>
