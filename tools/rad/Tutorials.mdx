...
- [CT stack](https://stack.chaitin.com/tool/detail/2)
...
![rad-1.png](/images/tools/rad/rad-1.png)

执行完成后应如下图

![rad-2.png](/images/tools/rad/rad-2.png)

### 三、基础使用（Basic）

首先使用`rad.exe -h`查看Rad本身自带的帮助信息

![rad-3.png](/images/tools/rad/rad-3.png)
...
3.  `--text-output`|`--text`
   ...
   - 输入结果如图：![rad-4.png](/images/tools/rad/rad-4.png)
4. `--full-text-output`|`--full`
    - 指定输出模式为txt，且将导出完整请求，也就是包括了请求头与请求体。
    - 输入结果如下图：![rad-5.png](/images/tools/rad/rad-5.png)
5. `--json-output`|`--json`
    - 指定输出模式为json，且将导出完整请求，也就是包括了请求头与请求体。
    - 输出结果如下图：![rad-6.png](/images/tools/rad/rad-6.png)
6. `--auto-index`|`--index`
    - 该参数需配合结果输出参数一起使用，他会检测到如果当前生成的结果文件已存在，便自动在结果文件的名称后面添加编号，不需要自己再修改生成文件的名称。
    - 效果如图：![rad-7.png](/images/tools/rad/rad-7.png)
7. `--http-proxy`
...
9. `--log-level`
    ...
    - 效果如下图：![rad-8.png](/images/tools/rad/rad-8.png)
10. `--no-banner`
    - 去掉banner信息
    - 效果如图：![rad-9.png](/images/tools/rad/rad-9.png)
11. `--wait-login`
    1. 使用该参数会自动禁用无头浏览模式，开启一个浏览器供手动登录。 在登录完毕后在命令行界面点击回车键继续爬取。注意需要登陆时请在配置文件中开启图片显示功能，否则会出现验证码不显示的现象
    2. 以下为登陆全流程
    3. 首先会开启一个空白界面，需要手动在地址栏输入要登陆的地址![rad-10.png](/images/tools/rad/rad-10.png)
    4. 响应完成后输入账号密码进行登录![rad-11.png](/images/tools/rad/rad-11.png)
    5. 登陆完成后进入主界面![rad-12.png](/images/tools/rad/rad-12.png)
    6. 这个时候回到shell界面点击回车即可继续爬取![rad-13.png](/images/tools/rad/rad-13.png)
...
- 使用burp作为上级代理，只需要将bp打开，然后将Rad的http-proxy设置为burp的代理端口即可![rad-14.png](/images/tools/rad/rad-14.png)

- burp的抓包打开后，就可以直接使rad暂停住，可以修改包的内容再发送。

- 也可以将intercept置为off，在HTTP history中查看发送与返回的数据包![rad-15.png](/images/tools/rad/rad-15.png)

- 如果需要再挂一层代理改变出网ip，只需要在Burp的User options中的Upstream Proxy Servers设置上层代理即可![rad-16.png](/images/tools/rad/rad-16.png)
...
1.  多条命令之间推荐使用`;`来执行，第一时因为linux的命令行与powershell都支持该符号分割，第二也是可以在上一条命令执行失败的情况下继续执行下一条命令，从而达到出现了问题也不影响后续的效果。
   - 效果如下图：![rad-17.png](/images/tools/rad/rad-17.png)
...
```
   ...
   - 这是目标文件：![rad-18.png](/images/tools/rad/rad-18.png)

   - 这是生成结果： ![rad-19.png](/images/tools/rad/rad-19.png)
...
-  `path_disallowed: []      # 不允许访问的路径, 支持的格式如: test、*test*`
    - 拿这个参数举例子：
    - 未修改时所爬取到的链接![rad-20.png](/images/tools/rad/rad-20.png)

    - 添加不想被爬的路径：![rad-21.png](/images/tools/rad/rad-21.png)

    - 修改后的爬取结果：![rad-22.png](/images/tools/rad/rad-22.png)
...
```
- 该脚本只是实例，可根据自己需求自行修改。
- 效果如下，为方便展示，将分号替换为回车：![rad-23.png](/images/tools/rad/rad-23.png)
